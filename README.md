...
- **🌙 다크 모드**: 라이트/다크 테마 전환 지원, 시스템 설정 연동
...
- **Theme**: next-themes 0.4.6 (다크 모드 지원)
...
│   │   ├── ThemeProvider.tsx         # 테마 컨텍스트 프로바이더
...
│   │   ├── ShareButton.tsx           # 공유 버튼 (소셜 미디어 공유)
...
#### 6. **다크 모드 구현**
- **Tailwind CSS v4 클래스 기반 다크 모드**:
  - `@variant dark (.dark &)` 문법 사용
  - `.dark` 클래스로 전환 (media query 미사용)
  - `globals.css`에서 CSS 변수 정의

- **next-themes 통합**:
  - `ThemeProvider`로 앱 전체 감싸기
  - `attribute="class"` 설정으로 클래스 기반 전환
  - `resolvedTheme` 사용으로 정확한 테마 감지
  - Hydration-safe 렌더링 (`mounted` 상태)

- **색상 전략 (Layered Contrast)**:
  ```css
  /* 페이지 배경 */
  ...
  text-gray-700 dark:text-gray-200     /* 레이블 */
  text-gray-600 dark:text-gray-300     /* 본문 */
  text-gray-500 dark:text-gray-400     /* 보조 */
  ```

- **컴포넌트별 다크 모드 적용**:
  - 모든 텍스트, 배경, 경계선에 `dark:` 변형 적용
  - 스켈레톤 UI에 `dark:bg-gray-700` 적용
  - 버튼, 모달, 인풋 필드 등 모든 인터랙티브 요소 지원
